// Target: C# 7.3 compatible (no nullable reference types)
...
namespace SCLOCUA
{
    // Simple lightweight guard to avoid dependency on external helpers
    internal static class Guard
    {
        // Ensures string is not null/whitespace; throws if invalid.
    ...
    public partial class killFeed : Form
    {
        ...
        // NOTE: Overridden property to avoid focus stealing
        ...
        public killFeed(string folderPath)
        {
            ...
            if (File.Exists(_wav))
            {
                ...
                try { _player.LoadAsync(); } catch { /* ignore sound load errors */ }
            }
            ...
            // Start tailing
            // NOTE: fire-and-forget; WinForms sync context will keep exceptions surfaced via MessageBox
            var _ = TailAsync();
        }
        ...
        private async Task TailAsync()
        {
            try
            {
                ...
                using (var sr = new StreamReader(fs))
                {
                    ...
                    while (!IsDisposed)
                    {
                        // NOTE: in C# 7.3, async ReadLineAsync is fine
        ...
        private void PlaySound()
        {
            ...
            catch { /* ignore sound play errors */ }
        }
        ...
        {
            // Simple DTO for parsed kill line
            public string T = string.Empty;
            public string Killer = string.Empty;
            public string Victim = string.Empty;
            public bool Suicide;
        }

        // NOTE: In C# 7.3 reference types can be null without annotations, so we keep "KillData" and return null when needed.
        private KillData Parse(string line)
        {
            ...
            Func<string, bool> IsNonPlayer = s =>
            {
                if (string.IsNullOrWhiteSpace(s)) return true; // treat empty as non-player
                for (int i = 0; i < nonPlayerTokens.Length; i++)
                {
                    var t = nonPlayerTokens[i];
            ...
            if (string.Equals(killer, "unknown", StringComparison.OrdinalIgnoreCase)) return null;
        ...
        {
            return !string.IsNullOrEmpty(name) && name.IndexOf("NPC", StringComparison.OrdinalIgnoreCase) < 0;
        }
        ...
        private sealed class KillLine : Control
        {
            ...
            private DateTime? _fadeStartUtc = null; // nullable DateTime is fine in C# 7.3 (value type)
            ...
            public static int MeasureHeight(Font f)
            {
                ...
                {
                    // Measure text height without padding; add vertical padding after
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                // build segments
                var list = new Tuple<string, Color>[] // null slots will be skipped
                {
            ...
            private static GraphicsPath BuildTextPath(Graphics g, Font f, string s, Point origin)
            {
                ...
                // Use DPI-aware em size for consistent rendering
            ...
            {
                // Use RectangleF + 3-arg AddArc(RectangleF, startAngle, sweepAngle)
                float d = rad * 2f;
                var gp = new GraphicsPath();

                gp.AddArc(new RectangleF(r.X, r.Y, d, d), 180f, 90f); // top-left
                gp.AddArc(new RectangleF(r.Right - d, r.Y, d, d), 270f, 90f); // top-right
                gp.AddArc(new RectangleF(r.Right - d, r.Bottom - d, d, d), 0f, 90f); // bottom-right
                gp.AddArc(new RectangleF(r.X, r.Bottom - d, d, d), 90f, 90f); // bottom-left
