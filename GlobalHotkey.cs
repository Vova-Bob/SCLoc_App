...
namespace ExecutiveHangarOverlay
{
    ...
    ///  - F7            -> set start NOW (local override)
    ...
    internal sealed class HotkeyMessageFilter : IMessageFilter, IDisposable
    {
        ...
        public HotkeyMessageFilter(IntPtr windowHandle)
        {
            ...
            RegisterHotKey(_windowHandle, ID_TOGGLE_OVERLAY, MOD_NONE, Keys.F6);
            RegisterHotKey(_windowHandle, ID_TOGGLE_CLICKTHR, MOD_NONE, Keys.F8);
            RegisterHotKey(_windowHandle, ID_TEMP_DRAG, MOD_CONTROL, Keys.F8);

            // Timing controls
            RegisterHotKey(_windowHandle, ID_SET_START_NOW, MOD_NONE, Keys.F7);
            RegisterHotKey(_windowHandle, ID_PROMPT_MANUAL, MOD_SHIFT, Keys.F7);
            RegisterHotKey(_windowHandle, ID_FORCE_SYNC, MOD_NONE, Keys.F9);
            RegisterHotKey(_windowHandle, ID_CLEAR_AND_SYNC, MOD_SHIFT, Keys.F9);
